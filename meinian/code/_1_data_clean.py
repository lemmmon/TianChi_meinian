...
def step_1():
    # 读取数据
    path = '..'
    train=pd.read_csv(path + '/data/meinian_round1_train_20180408.csv',sep=',',encoding='gbk',low_memory=False)
    test=pd.read_csv(path + '/data/meinian_round1_test_b_20180505.csv',sep=',',encoding='gbk',low_memory=False)
    data_part1=pd.read_csv(path + '/data/meinian_round1_data_part1_20180408.txt',sep='$',encoding='utf-8',low_memory=False)
    data_part2=pd.read_csv(path + '/data/meinian_round1_data_part2_20180408.txt',sep='$',encoding='utf-8',low_memory=False)
    ...
    table_id_group.to_csv(path + '/data/part_tabid_size.csv',encoding='utf-8')
    ...
    merge_part1_2.to_csv(path + '/data/merge_part1_2.csv',encoding='utf-8')
    #    print(merge_part1_2.head())
    del merge_part1_2
    
    time.sleep(10)
    print('------------------------重新读取数据merge_part1_2--------------------------')
    merge_part1_2=pd.read_csv(path + '/data/merge_part1_2.csv',sep=',',encoding='utf-8')
    
    # 删除掉一些出现次数低，缺失比例大的字段，保留超过阈值的特征
    def remain_feat(df,thresh=1):
        exclude_feats = []
        print('----------移除数据缺失多的字段-----------')
        ...
        return feats
    feats=remain_feat(merge_part1_2,thresh=1)
    
    
    merge_part1_2=merge_part1_2[feats]
    merge_part1_2.to_csv(path + '/data/merge_part1_2.csv')
    ...
    train.to_csv(path + '/data/train_set.csv',index=False,encoding='gbk')
    test.to_csv(path + '/data/test_set.csv',index=False,encoding='gbk')
